    ...
    $CheckResult = mysqli_query($conn, $searchQuery);
      ...
      if(mysqli_num_rows($CheckResult) > 0)
      {
        ?>
             ...
             <h1>Schedule List</h1>
            <form method="get" action="deleteschedule.php">
        ...
        <?php
            for($i=0;$i<mysqli_num_rows($CheckResult);$i++)
            {
              ...
              //echo  "<td><button type=\"submit\" class=\"btn btn-sm btn-info\" id=\"id\" name=\"id\" value=\"$RecRow[id]\">Update</button>&nbsp;</td>";
              echo  "<td><button type=\"submit\" class=\"btn btn-sm btn-danger\" id=\"id\" name=\"id\" value=\"$RecRow[id]\">Delete</button></td>";
