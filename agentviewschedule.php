...
<body>
  ...
  <section class="relative" style="top:42px">
    <div class="container-fluid">
      ...
      {
        $searchQuery = "SELECT * FROM tblvesselschedule WHERE status = '1' AND departuredate > DATE_ADD(CURRENT_DATE, INTERVAL 1 DAY) AND space != 0 ORDER BY departuredate, arrivaldate ASC";
      ...
      if(mysqli_num_rows($CheckResult) > 0)
      {
        ...
        <?php
            for($i=0;$i<mysqli_num_rows($CheckResult);$i++)
            {
              ...
              echo  "<td><button type=\"submit\" class=\"btn btn-sm btn-info\" id=\"id\" name=\"id\" value=\"$RecRow[id]\">Book</button>&nbsp;</td>";
                          
            }
      }
      else
      {
        echo "<script>alert('No Schedule Available'); location='index.php'; </script>";
      }
